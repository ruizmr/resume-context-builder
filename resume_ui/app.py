...
uploads_root = str(Path.home() / ".resume-context-uploads")
...
with st.sidebar:
	...
	instruction_file = st.text_input("Instruction file path (Markdown)", value=default_instr)
	...
	st.divider()
	st.caption("Instructions file (optional)")
	st.write("This Markdown is prepended to the packaged output to guide the LLM.")
	new_instr = st.text_area("Edit instructions (preview only)", value=(Path(default_instr).read_text(encoding="utf-8") if default_instr and os.path.isfile(default_instr) else ""), height=180)
	save_instr_path = st.text_input("Save instructions as", value=str((Path.home() / "instruction.md")))
	col_si1, col_si2 = st.columns(2)
	with col_si1:
		if st.button("Save instructions", use_container_width=True):
			try:
				p = Path(save_instr_path).expanduser().resolve()
				p.parent.mkdir(parents=True, exist_ok=True)
				p.write_text(new_instr or "", encoding="utf-8")
				st.success(f"Saved instructions to {p}")
			except Exception as e:
				st.error(f"Failed to save instructions: {e}")
	with col_si2:
		use_saved = st.button("Use saved instructions", use_container_width=True)
	if use_saved:
		instruction_file = save_instr_path
...
if start:
	...
	with st.spinner("Packaging context with Repomix..."):
		instr_path = instruction_file if (use_hr_instructions and instruction_file and os.path.isfile(instruction_file)) else None
