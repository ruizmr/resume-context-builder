...
def package_markdown_directory(
	...
	*,
	...
	# Split by tokens; include preamble only once
	chunks = _split_output_by_tokens(result.output_content, max_tokens=max_tokens, encoding_name=encoding_name, include_preamble_once=True)
	if len(chunks) <= 1:
		return [output_path]
